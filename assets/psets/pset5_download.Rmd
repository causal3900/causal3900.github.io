...
Relevant material will be covered by **Oct 30**. Problem set is due **Nov 11**.
...
\begin{tikzpicture}[x = .3in, y = .15in]
    ...
    \node (a) at (-2,0) {$A$};
    \node (y) at  (2,0) {$Y$};
    \node[font = \footnotesize, align = right, anchor = east] at (z.west) {Randomized to\\recieve encouragement};
    \node (u) at  (-2,-2) {$U$};
    \node[font = \footnotesize, align = center, anchor = south] at (a.north) {Attends\\office hours};
    \node[font = \footnotesize, align = left, anchor = west] at  (y.east) {Final Grade};
    \node[font = \footnotesize, align = right, anchor = east] at (u.west) {Interest in course};
